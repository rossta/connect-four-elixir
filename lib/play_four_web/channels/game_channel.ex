defmodule PlayFourWeb.GameChannel do
  ...
  def join("game:" <> game_id, payload, socket) do
    ...
    with {:ok, game_server} <- Games.Cache.server(game_id) do
      game = case Games.Server.join(game_server, player_id, socket.channel_pid) do
        {:ok, game} ->
  ...
  def handle_in("game:joined", _mesage, socket) do
    %{game_id: game_id} = socket.assigns

    with {:ok, game_server} <- Games.Cache.server(game_id),
         game <- Games.Server.game(game_server) do
  ...
  def handle_in("game:move", %{"col" => col}, socket) do
    ...
    with {:ok, game_server} <- Games.Cache.server(game_id),
         {:ok, game} <- Games.Server.move(game_server, player_id, col) do
  ...
  def handle_in("game:leave", _msg, socket) do
    ...
    {:ok, game_server} = Games.Cache.server(game_id)
    Games.Server.stop(game_server)
    {:noreply, socket}
  end
